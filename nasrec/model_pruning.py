...
def prune_elastic_layer(elastic_layer, num_neurons_to_prune):
    layer = elastic_layer._linear

    # Compute the L1-norm of the neuron weights
    l1_norms = layer.weight.data.norm(p=1, dim=1)
    neurons_to_prune = l1_norms.argsort()[:num_neurons_to_prune]

    # Create a new nn.Linear layer with pruned neurons
    pruned_layer = nn.Linear(layer.in_features, layer.out_features - num_neurons_to_prune)
    pruned_layer.weight.data = torch.index_select(layer.weight.data, 0, torch.tensor([i for i in range(layer.out_features) if i not in neurons_to_prune]))
    pruned_layer.bias.data = torch.index_select(layer.bias.data, 0, torch.tensor([i for i in range(layer.out_features) if i not in neurons_to_prune]))

    # Replace the nn.LazyLinear layer in the ElasticLinear module with the pruned nn.Linear layer
    elastic_layer._linear = pruned_layer
...
def train_and_eval_one_model(model, args):
    ...
    print("Dumping logs to {}!".format(args.logging_dir))
    save_model_checkpoint(
        model, os.path.join(args.logging_dir, "pruned_supernet_checkpoint.pt"), optimizer
    )
    dump_pickle_data(
        os.path.join(args.logging_dir, "train_test_logs.pickle"), epoch_logs
    )
...
def main(args):
    ...
    for idx, block in enumerate(model._blocks):
        choice = model.choice["micro"][idx]
    
        if choice["deep_fm"] == 0:
            for node in block._nodes:
                if isinstance(node, ElasticLinear):
                    prune_elastic_layer(node, num_neurons_to_prune=node._linear.out_features // 2)
                    if choice["dense_sparse_interact"]==1:
                        block.project_emb_dim=nn.Linear(384,128,bias=False)
                #elif isinstance(node, ElasticLinear3D):
                #    prune_elastic_layer(node, num_neurons_to_prune=node._linear.out_features // 2)
...
#export the pruned model to onnx format
    example_output = model(int_feats.to('cpu'),cat_feats.to('cpu'))
    
    torch.onnx.export(model,
            args=(int_feats.to('cpu'),cat_feats.to('cpu')),
        ...
        if choice["deep_fm"] == 0:
            for node in block._nodes:
                if isinstance(node, ElasticLinear) or isinstance(node, ElasticLinear3D):
                    ...
                    if choice["dense_sparse_interact"]==1:
                        block.project_emb_dim.to(0)
                        for param in block.project_emb_dim.parameters():
                            param.requires_grad = True
