...
class DotProduct(nn.Module):
    ...
    def custom_tril_indices(self, row, col, offset):
        rows = torch.arange(row).unsqueeze(1).expand(row, col)
        cols = torch.arange(col).unsqueeze(0).expand(row, col)
    
        mask = cols <= rows + offset
        rows_indices, cols_indices = torch.where(mask)
    
        return rows_indices, cols_indices
    ...
    def forward(self, dense_t: torch.Tensor, sparse_t: torch.Tensor, dims_in_use: int):
        ...
        # offset = 0
        li, lj = self.custom_tril_indices(ni, nj, offset)
        Zflat = Z[:, li, lj]
