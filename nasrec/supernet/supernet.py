...
class SuperNetBlock(nn.Module):
    ...
    def forward(self, tensors, choices=None):
        ...
        if choice['dense_sparse_interact'] == 1:
            ...
            if dense_t_2d_out_proj.size(-1) != self._embedding_dim * DS_INTERACT_NUM_SPLITS:
                zeros_tensor_size = torch.Size(
                    [sparse_t_3d.size(0), ((DS_INTERACT_NUM_SPLITS*self._embedding_dim)-dense_t_2d_out_proj.size(-1))])
                zero_pad = _zeros_generator(zeros_tensor_size, dense_t_2d_out.device) 
                dense_t_2d_out_proj = torch.cat([dense_t_2d_out_proj,zero_pad],dim=1)
            dense_t_2d_out_proj = dense_t_2d_out_proj.view([dense_t_2d_out_proj.size(0), -1, self._embedding_dim])
    ...
    def fixed_forward(self, tensors, choices):
        ...
        if choice['dense_sparse_interact'] == 1:
            if dense_t_2d_out.size(-1) != self._embedding_dim * DS_INTERACT_NUM_SPLITS and self.project_emb_dim is not None:
                ...
                self.project_emb_dim, self.project_emb_dim_layernorm = None, None
                dense_t_2d_out_proj = dense_t_2d_out
            if dense_t_2d_out.size(-1) != self._embedding_dim * DS_INTERACT_NUM_SPLITS and self.project_emb_dim is None:
                
                dense_t_2d_out_proj = dense_t_2d_out.clone()
                zeros_tensor_size = torch.Size(
                    [sparse_t_3d.size(0), abs((DS_INTERACT_NUM_SPLITS*self._embedding_dim)-dense_t_2d_out_proj.size(-1))])
                zero_pad = _zeros_generator(zeros_tensor_size, dense_t_2d_out.device) 
                dense_t_2d_out_proj = torch.cat([dense_t_2d_out_proj,zero_pad],dim=1)
