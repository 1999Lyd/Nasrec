...
from NasRec.dlrm.prune_module import Projected_MaskedLinear
...
def l2_regularization(model, scale=1e-6):
    norm = 0
    for i,layer in enumerate(model.dense_arch.all_modules):
        

        if isinstance (layer, Projected_MaskedLinear):
            hidden_state_1 = layer.blocks[0](layer.weight .t())
            hidden_state_2 = layer.blocks[1](hidden_state_1.t())
            mask = layer.blocks[2](hidden_state_2.t())
            l2 = torch.norm(mask, 2)**2
            norm += l2
    for i,layer in enumerate(model.over_arch.all_modules):
        if isinstance (layer, Projected_MaskedLinear):
            hidden_state_1 = layer.blocks[0](layer.weight .t())
            hidden_state_2 = layer.blocks[1](hidden_state_1.t())
            mask = layer.blocks[2](hidden_state_2.t())
...
def l1_regularization(model, scale=1e-4):
    norm = 0
    for i,layer in enumerate(model.dense_arch.all_modules):
...
def intra_neuron_similarity(model, scale=0.005):
    # pairwise squared difference for each neuron
    loss = 0
    for i,layer in enumerate(model.dense_arch.all_modules):
...
def inter_neuron_dissimilarity(model, scale=0.005):
    # mean mask value for each neuron
    loss = 0
    for i,layer in enumerate(model.dense_arch.all_modules):
        ...
        if isinstance (layer, Projected_MaskedLinear):
            ...
            #hidden_state_2 = layer.blocks[1](hidden_state_1.t())
            mask = layer.blocks[1](hidden_state_1.t())
            mu = mask.mean(dim=1, keepdim=True)
    # pairwise absolute difference of means
            diff = torch.abs(mu - mu.t())
            loss += diff.mean()
...
def train_and_test_one_epoch(
    ...
    for batch_num, (int_x, cat_x, y) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            ...
            #mask_l2 = l2_regularization(model)
            #intra_loss = intra_neuron_similarity(model)
            #inter_loss = inter_neuron_dissimilarity(model)
            total_loss = loss + l2_loss#+intra_loss+inter_loss
        ...
        # Log gradient norm to TensorBoard
                    if tb_writer is not None:
                        tb_writer.add_scalar('Gradient Norm {}'.format(name), grad_norm, batch_num)
                        if batch_num%5 == 0:
                            tb_writer.add_histogram(f'{name}.grad', param.grad, global_step=batch_num)
                '''
                if grad_clip_value is not None:
                    torch.nn.utils.clip_grad_norm_(model.parameters(), grad_clip_value)
                optimizer.step()
        ...
        if batch_num % test_interval == 0 or batch_num == max_train_steps - 1:
            if (not test_only_at_last_step) or (
                ...
                #train_acc, train_auroc, train_loss = test_one_epoch(
                #    model, train_loader, loss_fn, gpu, max_steps=max_eval_steps, use_amp=use_amp
                #)
                ...
                #print(
                #    "Train Acc: {}, Train AUROC: {}, Train Loss: {}".format(
                #        train_acc, train_auroc, train_loss
                #    )
                #)
