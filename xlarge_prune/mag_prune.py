...
from nasrec.supernet.modules import ElasticLinear,ElasticLinear3D,SigmoidGating
...
class Projected_MaskedLinear(nn.Module):
    def __init__(self, in_features, out_features, device, bias=True):
        ...
        self.projector = nn.Linear(out_features, out_features, bias=False)
    ...
    def forward(self, input):
        mask = torch.sigmoid(self.projector(self.weight.t())).t()
...
def mask_elastic_layer(elastic_layer):
    ...
    masked_layer = Projected_MaskedLinear(layer.in_features, layer.out_features, device='cuda:1')
...
def prune_layer(elastic_layer):
    with torch.no_grad():
        layer = elastic_layer._linear
        
        
        neuron_scores = layer.weight.data.abs().sum(dim=1)  # sum inside each neuron
        #new_scores = neuron_scores[neuron_scores!=0]
        sorted_scores, _ = torch.sort(neuron_scores)
        print(len(sorted_scores))
        index = int(len(sorted_scores) * (50/ 100))
        print(index)
...
def main(args):
    ...
    checkpoint = torch.load("supernet-config_checkpoint.pt",map_location='cuda:1')

    model_state_dict = checkpoint['model_state_dict']

    model_trained.load_state_dict(model_state_dict, strict=True)

    init_state_dict = torch.load("initial_weights.pth",map_location='cuda:1')
    keys_to_remove = [k for k in init_state_dict.keys() if 'projector' in k]
    ...
    for idx, block in enumerate(model_trained._blocks):
        ...
        if block.project_emb_dim is not None:
            ...
            block.project_emb_dim,keep_indices = prune_linear(block.project_emb_dim)
        ...
        for node in block._nodes:
            ...
            if isinstance(node, SigmoidGating):
                if node._left_self_linear._linear is not None:
                    ...
                    ori_output[counter] = out_feats
                    node._left_self_linear._linear,keep_indices = prune_linear(node._left_self_linear._linear)
                    indices[counter] = keep_indices
                ...
                if node._linear_proj is not None:
                    ...
                    ori_output[counter] = out_feats
                    node._linear_proj,keep_indices = prune_linear(node._linear_proj)
                    indices[counter] = keep_indices
            '''      
            if isinstance(node, ElasticLinear):
                out_feats = node._linear.out_features
                ori_output[counter] = out_feats
                node._linear,keep_indices = prune_layer(node)
                indices[counter] = keep_indices
            '''
    counter = 0
