...
def get_model(args):
    if args.net == "dlrm":
        ...
        model = DLRM(
            ...
            over_arch_dims=[1024,1024,512,512,256,256],
...
def train_and_eval_one_model(model, indices, args):
    ...
    #torch.save(model,os.path.join(args.logging_dir, "{}_checkpoint.pt".format(args.net)))
    save_model_checkpoint(
...
def main(args):
    ...
    for idx, block in enumerate(model_trained._blocks):
        ...
        if block.project_emb_dim is not None:
            layer = block.project_emb_dim
            masked_layer = Projected_MaskedLinear(layer.in_features, layer.out_features, args.gpu)
            masked_layer.weight.data = layer.weight.data
            masked_layer.bias.data = layer.bias.data
            block.project_emb_dim = masked_layer
    ...
    checkpoint = torch.load("supernet-config_checkpoint.pt",map_location='cuda:5')

    model_state_dict = checkpoint['model_state_dict']

    model_trained.load_state_dict(model_state_dict, strict=True)

    init_state_dict = torch.load("initial_weights.pth",map_location='cuda:5')
    ...
    for idx, block in enumerate(model_trained._blocks):
        ...
        for node in block._nodes:
            ...
            if isinstance(node, ElasticLinear3D):
                ...
                new_out = len(indices[counter])
                print(ori_out,new_out)
                node._linear = CustomLinear3D(in_feats,new_out, ori_out, indices[counter],device = args.gpu)
                node._linear.weight.data = pruned_weight
                node._linear.bias.data = pruned_bias
