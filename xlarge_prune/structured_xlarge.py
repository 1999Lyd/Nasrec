...
def train_and_eval_one_model(model, args):
    ...
    input_ = (int_x,cat_x)
    latency = get_model_latency(model,input_,args.gpu)
    ...
    int_feats = torch.randn((1, 13), dtype=torch.float).to('cuda:2')
    cat_feats = torch.randint(low=0, high=3, size=(1, 26), dtype=torch.long).to('cuda:2')
...
def prune_layer(elastic_layer,device):
    with torch.no_grad():
        ...
        index = int(len(sorted_scores) * (90/ 100))
        print(index)
        # Get the indices of the neurons to keep (above median score)
        threshold = sorted_scores[index]
        keep_indices = (neuron_scores >= threshold).nonzero().squeeze().to('cpu')
...
class CleverZeroTensorGenerator:
    ...
    def __call__(
        ...
        if token in self.cached_zeros:
            return self.cached_zeros[token]
        else:
            zeros = torch.zeros(size, dtype=torch.float, device=device)
            zeros.requires_grad = False
            self.cached_zeros[token] = zeros
            return zeros

class CustomLinear2D(nn.Module):
    def __init__(self, in_features, out_features, ori_out_features, zero_generator):
        super(CustomLinear2D, self).__init__()
        self.weight = nn.Parameter(torch.Tensor(out_features, in_features))
        self.bias = nn.Parameter(torch.Tensor(out_features))
        self.ori_out_features = ori_out_features
        self.zero_generator = zero_generator

    def forward(self, input):
        output = nn.functional.linear(input, self.weight, self.bias)

        # Use zero_generator for padding
        num_pad_zeros = self.ori_out_features - output.shape[1]
        if num_pad_zeros > 0:
            padding = self.zero_generator((output.shape[0], num_pad_zeros), device=output.device)
            output_full = torch.cat([output, padding], dim=1)
        else:
            output_full = output
...
class CustomLinear3D(nn.Module):
    ...
    def forward(self, input):
        ...
        # Use zero_generator for padding in 3D
        num_pad_zeros = self.ori_out_features - output.shape[2]
        if num_pad_zeros > 0:
            padding = self.zero_generator((output.shape[0], output.shape[1], num_pad_zeros), device=output.device)
            output_full = torch.cat([output, padding], dim=2)
        else:
            output_full = output
...
def main(args):
    ...
    sparse_offset = {}
    counter = 0
    check_list = model_init.choice['macro']
    ori_dim = model_init.choice['micro']
    for idx, block in enumerate(model_trained._blocks):
        ...
        for node in block._nodes:
            ...
            if isinstance(node, SigmoidGating):
                ...
                if node._linear_proj is not None:
                    ...
                    indices[counter] = keep_indices
                    dense_list[idx+1] = keep_indices
                    dense_offset[idx+1] = out_feats
            ...
            if isinstance(node, ElasticLinear3D):
                ...
                node._linear,keep_indices = prune_layer(node,args.gpu)
                indices[counter] = keep_indices
                sparse_list[idx+1] = keep_indices
                if ori_dim[idx]['dense_sparse_interact']==0:
                    sparse_offset[idx+1] = out_feats
                    sparse_list[idx+1] = keep_indices
                else:
                    sparse_offset[idx+1] = out_feats+8
                    cat_idx = torch.arange(out_feats,out_feats+8)
                    sparse_list[idx+1] = torch.cat([keep_indices,cat_idx])

    counter = 0
    dense_list[0] = torch.arange(13)
    dense_offset[0] = 13
    sparse_list[0] = torch.arange(26)
    sparse_offset[0] = 26
    for idx, block in enumerate(model_init._blocks):

        block_connection = check_list[idx]
        dense_connection = block_connection['dense_idx']
        sparse_connection = block_connection['sparse_idx']
        dense_left_connection = block_connection['dense_left_idx']
        dense_right_connection = block_connection['dense_right_idx']
        
        offset = 0
        dense_indices = []
        for idx in dense_connection:
            if idx in dense_list.keys():
                index = [x+offset for x in dense_list[idx]]
                dense_indices.extend(index)
                offset+=dense_offset[idx]
            
            else:
                output_dim = ori_dim[idx]['dense_in_dims']
                ind = torch.arange(output_dim)
                index = [x+offset for x in ind]
                dense_indices.extend(index)
                offset+=output_dim
        ...
        offset = 0
        sparse_indices = []
        for idx in sparse_connection:
            if idx in sparse_list.keys():

                index = [x+offset for x in sparse_list[idx]]
                sparse_indices.extend(index)
                offset+=sparse_offset[idx]
            else:
                output_dim = ori_dim[idx]['sparse_in_dims']
                ind = torch.arange(output_dim)
                index = [x+offset for x in ind]
                sparse_indices.extend(index)
                offset+=output_dim
        ...
        offset = 0
        dense_left_indices = []
        for idx in dense_left_connection:
            if idx in dense_list.keys():
                index = [x+offset for x in dense_list[idx]]
                dense_left_indices.extend(index)
                offset+=dense_offset[idx]
        ...
        offset = 0
        dense_right_indices = []
        for idx in dense_right_connection:
            if idx in dense_list.keys():
                index = [x+offset for x in dense_list[idx]]
                dense_right_indices.extend(index)
                offset+=dense_offset[idx]
        ...
        for node in block._nodes:
            ...
            if isinstance(node, SigmoidGating):
                ...
                if node._linear_proj is not None:
                    counter+=1
                    if len(dense_left_indices)>=len(dense_right_indices):
                        
                        selected_rows = node._linear_proj.weight.data[indices[counter],:] 
                        pruned_weight = selected_rows[:,dense_left_indices]
                        pruned_bias = node._linear_proj.bias.data[indices[counter]]
                        in_feats = len(dense_left_indices)
                        new_out = len(indices[counter])
                        ...
                        selected_rows = node._linear_proj.weight.data[indices[counter],:] 
                        pruned_weight = selected_rows[:,dense_right_indices]
                        pruned_bias = node._linear_proj.bias.data[indices[counter]]
                        in_feats = len(dense_right_indices)
                        new_out = len(indices[counter])
                        node._linear_proj = nn.Linear(in_feats,new_out)
                        node._linear_proj.weight.data = pruned_weight
                        node._linear_proj.bias.data = pruned_bias
            ...
            if isinstance(node, DotProduct):
                counter+=1
                right_len = len(dense_right_indices)
                in_feats = ((right_len+1)*right_len)/2
                in_feats = int(in_feats)
                selected_rows = node._linear_proj.weight.data[indices[counter],:] 
                pruned_weight = selected_rows[:, :in_feats]
                pruned_bias = node._linear_proj.bias.data[indices[counter]]
                ...
                pruned_weight = node._dense_proj.weight.data[:,dense_left_indices]
                pruned_bias = node._dense_proj.bias.data
                left_len = len(dense_left_indices)
                ori_out = node._dense_proj.out_features
                node._dense_proj = nn.Linear(left_len,ori_out)
                node._dense_proj.weight.data = pruned_weight
                node._dense_proj.bias.data = pruned_bias
            
            
            
            if isinstance(node, ElasticLinear):
                counter+=1
                selected_rows = node._linear.weight.data[indices[counter], :]
                pruned_weight = selected_rows[:, dense_indices]
                pruned_bias = node._linear.bias.data[indices[counter]]
                
                in_feats = len(dense_indices)
            ...
            if isinstance(node, ElasticLinear3D):
                ...
                selected_rows = node._linear.weight.data[indices[counter], :]
                pruned_weight = selected_rows[:, sparse_indices]
 
                pruned_bias = node._linear.bias.data[indices[counter]]
                
                in_feats = len(sparse_indices)
    ...
    counter = 0
    zero_generator = CleverZeroTensorGenerator()
    for idx, block in enumerate(model_trained._blocks):
        ...
        if block.project_emb_dim is not None:
            ...
            block.project_emb_dim = CustomLinear2D(in_feats,new_out, ori_out, zero_generator)
            block.project_emb_dim.weight.data = pruned_weight
            block.project_emb_dim.bias.data = pruned_bias
        ...
        for node in block._nodes:
            ...
            if isinstance(node, SigmoidGating):
                if node._left_self_linear._linear is not None:
                    ...
                    new_out = len(indices[counter])
                    print(ori_out,new_out)
                    node._left_self_linear._linear = CustomLinear2D(in_feats,new_out, ori_out, zero_generator)
                ...
                if node._linear_proj is not None:    
                    ...
                    new_out = len(indices[counter])
                    print(ori_out,new_out)
                    node._linear_proj = CustomLinear2D(in_feats,new_out, ori_out, zero_generator)
                    node._linear_proj.weight.data = pruned_weight
                    node._linear_proj.bias.data = pruned_bias
            ...
            if isinstance(node, ElasticLinear):
                ...
                new_out = len(indices[counter])
                print(ori_out,new_out)
                node._linear = CustomLinear2D(in_feats,new_out, ori_out, zero_generator)
                node._linear.weight.data = pruned_weight
                node._linear.bias.data = pruned_bias
            ...
            if isinstance(node, ElasticLinear3D):
                ...
                new_out = len(indices[counter])
                print(ori_out,new_out)
                node._linear = CustomLinear3D(in_feats,new_out, ori_out, zero_generator)
                node._linear.weight.data = pruned_weight
                node._linear.bias.data = pruned_bias
    '''
    final_layer = model_init._final
    dense_idx = dense_list[7]
    offset = dense_offset[7]
    sparse_idx = torch.arange(48)
    sparse = [x+offset for x in sparse_idx]
    indices = torch.cat([torch.tensor(dense_idx),torch.tensor(sparse)])
    in_dim = offset+48
    pruned_weight = final_layer.weight.data[:,indices]
    pruned_bias = final_layer.bias.data
    model_init._final = nn.Linear(in_dim,1)
    model_init._final.weight.data = pruned_weight
    model_init._final.bias.data = pruned_bias
